...
import com.ok.mqtt.extension.parcelable
import com.ok.mqtt.extension.serializable
...
class MqttAndroidClient @JvmOverloads constructor(
    ...
    private fun onReceive(data: Bundle) {
        ...
        if (status == Status.ERROR) {
            Log.e(TAG, "$status $data")
        } else if (data.toString().lowercase().contentEquals("exception"))
            Log.w(TAG, "$status $data")
        else
            Log.v(TAG, "$status $data")
    ...
    private fun storeToken(token: IMqttToken?) {
        token?.let {
            ...
            if (tokenFound == null) {
                tokenList.add(token)
                Log.d(TAG, "$token size=${tokenList.size}")
            }
        }
    }
    ...
    private fun removeMqttToken(data: Bundle): IMqttToken? {
        ...
        if (activityToken != null && activityToken != "null") {
            ...
            Log.d(TAG, "search=$activityToken ${tokenList.size}")
            return token
        }
        return null
    }
    ...
    companion object {
        private const val TAG = "MqttAndroidClient"
